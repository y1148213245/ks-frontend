      ...
      <div class="twowrap">
        ...
        <div class="searchCon">
          ...
          <div class="searchBox">
            ...
            <input type="text" id="searchText" class="searchText" value="" placeholder="输入关键字" v-model="searchText" @keyup.13="goToSearchResult($event)">
          </div>
  ...
  export default {
    ...
    methods: {
      ...
      goToAdvSearch: function () {
        window.location.href = '../pages/searchadvanced.html'
      },
      goToSearchResult () {
...
//        window.location.href = '../pages/search.html?searchText=' + this.searchText + "&category=" + this.category + "#";
      ...
      returnUrl: function (arg) {
        let urlParse = URL.parse(document.URL, true);
        urlParse.pathname = arg.url ? arg.url : urlParse.pathname;
        if (arg.replace) {
          urlParse.query = arg.param ? arg.param : urlParse.query;
        } else {
          urlParse.query = Object.assign(urlParse.query, arg.param);
        }
        urlParse.search = "";
        window.location.href = URL.format(urlParse);
      },
