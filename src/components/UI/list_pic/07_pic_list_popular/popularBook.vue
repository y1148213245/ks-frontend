    ...
    <dl class="listDl" v-for="(entry,index) in popularBookList" :class="{floatLeft:index%2==0}" v-if="index<number" :key="index">
      <dt class="listDt">
        <a @click="toBookDetail(entry.pubId)" class="imgBox">
          <img class="popular_list_img" :src="entry.bookUrl"  onload="DrawImage(this,144,194)" :alt="getStaticText('noCover') ? getStaticText('noCover') : '暂无封面'"/>
        </a>
      </dt>
      <dd class="listDd">
        <p class="title">
        ...
        <p class="author">
          <span class="authorText">{{getStaticText('author') ? getStaticText('author') : '作者：'}}</span>
        ...
        <p class="other">
          ...
          <a href="javascript:void(0)" @click="toBookDetail(entry.pubId)" class="chakan f12 color_fff">{{getStaticText('see') ? getStaticText('see') : '查看'}}</a>
        </p>
      </dd>
    </dl>
  </div>
      ...
      Post(CONFIG.BASE_URL+this.CONFIG.url, this.CONFIG.param).then((rep) => {
        ...
        if (datas) {
          for (let i = 0; i < datas.length; i++) {
            let entry = {
              bookName: datas[i].pub_resource_name || (this.getStaticText('noBookName') ? this.getStaticText('noBookName') : '暂无书名') ,
              starNum: datas[i].pub_star_num,
              bookProdPrice: datas[i].prod_member_price || 0,
              bookPrice: datas[i].BOOK_EB_PRICE || 0,
              bookAuthor: datas[i].BOOK_SYS_AUTHORS || (this.getStaticText('noAuthor') ? this.getStaticText('noAuthor') : '暂无作者'),
              pubId: datas[i].id || 0,
              ...
              BOOK_PUBDATE: datas[i].BOOK_PUBDATE ? this.formatDate(datas[i].BOOK_PUBDATE, "yyyy-MM-dd") : (this.getStaticText('noDate') ? this.getStaticText('noDate') : "暂无日期")
            }
