 ...
 * @Last Modified time: 2018-09-07 18:25:10
 * 国际化组件
 */
 <template>
  <div class="ui_international_01">
    <span class="ui_international_01_langs" v-for="(lang, key, index) in langsConfig" :key="index" @click="toggleLangs(lang)" :class="{activeLangs: activeIndex == lang.order}">
...
export default {
  ...
  mounted () {
    ...
    this.langsConfig = this.CONFIG.langsConfig;
    this.i18nObj = JSON.parse(window.localStorage.getItem('i18n'));
    this.activeIndex = this.i18nObj.order;
  },
  methods: {
    toggleLangs (item) { // 切换语种 将国际化信息存入浏览器
      ...
      var curHtml = this.i18nObj.lang == 'cn' ? tempHtml : tempHtml.replace(this.i18nObj.hrefTag, '');
      let newUrl = curUrl.replace(tempHtml, curHtml + item.hrefTag);
