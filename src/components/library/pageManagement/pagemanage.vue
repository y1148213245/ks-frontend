  ...
  <div class="components_pagemanagement">
        ...
        <div class="listNav">
          <el-tabs type="border-card" v-model="activeType" @tab-click="toggleListType">
            <el-tab-pane v-for="(list, index) in listArray" :label="list.name" :key="index" :name="list.type">
              <div class="listCon">
                <div class="listHead">
                  <span class="operation" @click="createNew()">
                    <i class="el-icon-plus"></i>
                    <span>新建</span>
                  </span>
                  <span class="operation">
                    <el-upload class="upload-demo" :action="upLoadUrl()" name="file" :show-file-list="false" :on-progress="upLoading" :on-success="upLoadingSuccess" :before-upload="beforeUpload">
                      <i class="el-icon-upload2"></i>
                      <span>上传</span>
                    </el-upload>
                  </span>
                  <span class="operation">
                    <el-upload class="upload-demo" :action="importUrl()" name="file" :show-file-list="false" :on-progress="importProgress" :on-success="importSuccess" :before-upload="beforeImport">
                      <i class="el-icon-upload"></i>
                      <span>导入</span>
                    </el-upload>
                  </span>
                  <span class="operation" @click="downloadFile()">
                    <i class="el-icon-download"></i>
                    <span>导出</span>
                  </span>
                  <span class="operation" @click="deleteFile()">
      ...
      <div class="mainContent">
        <div class="reviewArea" v-if="debugModel">
          <div class="reviewAreaDiv">
            <el-upload class="upload-demo" :action="importResource()" name="file" :show-file-list="false" :on-progress="importProgress" :on-success="importResourceSuccess" :before-upload="beforeImport">
              <i class="el-icon-upload"></i>
              <span>更新全部站点组件库</span>
            </el-upload>
          </div>
          <div class="reviewAreaDiv">
            <el-upload class="upload-demo" :action="importProjectResource()" name="file" :show-file-list="false" :on-progress="importProgress" :on-success="importResourceSuccess" :before-upload="beforeImport">
              <i class="el-icon-upload"></i>
              <span>更新当前站点组件库</span>
            </el-upload>
          </div>
    ...
    <el-dialog title="新建文件" :visible.sync="createNewModel">
      <div>
        ...
        <span>文件名：</span>
        <input class="newFileName" v-model="newFileName" placeholder="请输入文件名称" @keyup.enter="confirmCreateNew()" />
        <span>.html</span>
      </div>
...
export default {
  ...
  methods: {
    ...
    showConfig (com) { // 显示当前组件的配置文件 支持编辑
      ...
      /*
       获取组件配置优先级： config/index.js $_$变量  >>  js/example.js
    ...
    confirmConfig (data) {  // 确定修改组件配置信息
      ...
      Post(this.configUrl + 'project/config', params).then((res) => {
        ...
        if (res.data && res.data.success) {
          ...
          this.reloadPageManagementConfig(); // 修改成功之后需要重新请求一次配置 解决缓存问题
        } else {
          this.$message({
            type: "info",
            message: "修改失败，请稍后重试"
    ...
    reloadPageManagementConfig () { // 重新请求页面管理系统的配置文件 config/index.js
      var oldChild = document.getElementById("pagemanagement_config");
      var scriptTag = document.createElement("script");
      scriptTag.type = "text/javascript";
      scriptTag.src = "../assets/config/index.js";
      scriptTag.id = "pagemanagement_config";
      oldChild.parentNode.replaceChild(scriptTag, oldChild);
    },
        ...
        Get(this.configUrl + 'files/edit?fileName=' + this.activeFile + '&projectName=' + this.siteName).then((res) => {
          ...
          if (datas.success && datas.content) {
            ...
            for (var i = 0; i < uiComponent.length; i++) {
              uiComArray.push(uiComponent[i].substring(uiComponent[i].lastIndexOf('</'), uiComponent[i].length).replace('</', '').replace('>', ''));
            }
            ...
            for (var j = 0; j < workComponent.length; j++) {
              workComArray.push(workComponent[j].substring(workComponent[j].lastIndexOf('</'), workComponent[j].length).replace('</', '').replace('>', ''));
            }
    ...
    deleteFile () {  // 删除文件
      this.$confirm(`删除后不可恢复，您确定要删除${this.activeFile}文件吗？`, "系统提示", {
    ...
    saveCode (status) { // 保存修改
      ...
      let params = {
        ...
        fileContent: window.btoa(encodeURIComponent(data.fileContent)),
        projectName: this.siteName
      }
...
.components_pagemanagement .newFileName {
  display: inline-block;
  width: 160px;
  background-color: #fff;
  border-radius: 4px;
