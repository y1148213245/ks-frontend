...
<script>
  ...
  import purchasedtwo from "@components/work_mobile/personalcenter/11_purchasedtwo/purchasedtwo.vue";
  import manageaddress from "@components/work_mobile/personalcenter/12_manageAddress/manageAddress.vue";
  import mycollection from "@components/work_mobile/personalcenter/14_mycollection/mycollection";
  import myorderlist from "@components/work_mobile/personalcenter/15_myorderlist/myorderlist";
  export default {
    ...
    components: {
      ...
      mycollection, //我的收藏带tab
      myorderlist //我的订单
    },

    created() {
      var getHash = URL.parse(window.location.href, true).hash;
      this.currentShowTag = getHash
        ? getHash.substring(getHash.indexOf("#") + 1, getHash.length)
        : "";
      this.getMemberInfo();
    },
