<template>
  <div>
    ...
    <div class="work_mobile_bookdetail_01_free_read">
      <el-button @click="toRead(bookInfo.resourceId, bookInfo.bookFreeDownLoadPath)" :class="{work_mobile_bookdetail_01_noProbation: !probation}">免费试读</el-button>
    </div>
...
export default {
  ...
  methods: {
    loadData () {
      var paramsObj = Object.assign({}, this.CONFIG.params);
      paramsObj.pubId = this.pubId;
      Get(CONFIG.BASE_URL + this.CONFIG.url, { params: paramsObj }).then((resp) => {
        this.bookInfo = resp.data.data;
        this.probation = this.bookInfo.bookFreeDownLoadPath ? true : false;
      })

    },
    toRead (bookId, probationUrl) {
      if (!probationUrl) {  // 没有试读地址的情况
        return false;
      }
      let readType = this.CONFIG.readType ? this.CONFIG.readType : 0;
