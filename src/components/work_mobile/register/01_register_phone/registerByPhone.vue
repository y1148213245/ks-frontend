 ...
 */
 <template>
  <div class="ui_work_register_01">
    ...
    <!-- 设置密码界面 -->
    <div class="ui_work_register_01_second" v-if="showItem == 'setPassword'">
      <van-field class="ui_work_register_01_password" v-model="password" :placeholder="display.setPassword" icon="clear" @click-icon="password = ''" @blur="formatPassword()" type="tel" maxlength="16" />
      <div class="ui_work_register_01_errorpassword" v-text="errorPassword" v-show="isWrongPassword"></div>
      <div class="ui_work_register_01_confirm">
...
export default {
  ...
  created () {
    this.CONFIG = PROJECT_CONFIG[this.namespace].work_mobile_register.work_mobile_register_01;
  ...
  methods: {
    ...
    checkExistPhoneNumber (flag) { // 校验手机号是否已经注册过
      ...
      Get(CONFIG.BASE_URL + setConfig.url, { 'params': paramsObj }).then((resp) => { // 验证用户信息 查寻手机号是否已经注册过
        let res = resp.data;
        if (res.result == '1') { // 手机号可以使用 即 手机号没有注册过 才去调取发送验证码的操作
    ...
    nextStep () { // 下一步
      this.formatPhoneNumber(false); // 单纯校验手机号码的问题  不需要调发送验证码的接口 所以传 false
...
.ui_work_register_01_errormsg {
  color: red;
}

.ui_work_register_01_errorpassword {
  color: red;
}

.ui_work_register_01_sendcheck {
  color: #35b8e4;
  border: none;
  background-color: #fff;
}

.ui_work_register_01_resendcheck {
  color: #35b8e4;
  border: none;
  background-color: #fff;
}
