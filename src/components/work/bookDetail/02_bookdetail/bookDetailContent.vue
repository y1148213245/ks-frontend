...
<template>
  <div class="work_bookdetail_02 work_bookdetail_02_skin">
    ...
    <!-- 音频附件 -->
    <div class="work_bookdetail_02-audio" v-if="CONFIG && CONFIG.showItem && CONFIG.showItem.indexOf('audio') != -1">
			<h4 class="work_bookdetail_02-audio-title" v-text="currentAudio && currentAudio.attachName"></h4>
			<br>
			<audio :src="currentAudio && currentAudio.url || ''" controls controlslist="nodownload"></audio><br/>
      <ul class="work_bookdetail_02-audio-list">
        <li v-for="(item,index) in (resourceDetail.audio || [])" :key="index"  @click="startAudio(item,resourceDetail.pub_audio,index)">
          <i class="el-icon-service"></i><span v-text="item.attachName"></span>
        </li>
      </ul>
		</div>
  </div>
...
export default {
  ...
  created: function () {
    this.getMemberInfo().then((member) => {
      ...
      params.attachTypes = 'audio'
      this.$store.dispatch('bookDetail/' + type.PRODB_BOOK_DETAIL, params);
  ...
  computed: {
    ...mapGetters({
      ...
      resourceDetail: 'bookDetail/prodbDetail'
    }),
    ...mapGetters("login", {
...
.work_bookdetail_02-audio {
  font-size: 14px;
  padding-left: 20px;
}
.work_bookdetail_02-audio-list {
  padding: 10px 0;
}
.work_bookdetail_02-audio-list li {
  cursor: pointer;
}
