...
export default {
  ...
  viewBookDetailInfo (params, cb) {
    ...
    axios.get(url).then(function (response) {
      if (response.data.result == "1" && cb && cb instanceof Function) {
        ...
        document.getElementsByClassName("work_bookdetail_02_share_more")[0].appendChild(document.createElement('script')).src =
  ...
  getProdbDetail (params, cb) {
    let url = CONFIG.BASE_URL + 'resource/detail.do'
    axios.get(url,{params}).then(resp => {
      // console.log(resp.data)
      if (resp.data.data && cb && cb instanceof Function) {
        cb({
          bookDetailInfo: resp.data.data,
        });
        /* loading.close(); */
      }
    })
