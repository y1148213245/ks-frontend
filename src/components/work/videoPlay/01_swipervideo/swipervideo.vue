 ...
 * @Last Modified time: 2018-11-22 18:19:04
 * 视频播放组件 列表是轮播图形式的
 */
...
export default {
  ...
  created() {
    this.CONFIG =
      PROJECT_CONFIG[this.namespace].work_videoplay.work_videoplay_01[
        this.modulename
      ];
  ...
  mounted() {
    ...
    script.onload = () => {
      if (this.CONFIG.queryParamsType == "url") {
        // 从视频组里取数据的时候才把参数传到地址栏里
        this.curId = queryObj && queryObj.id ? queryObj.id : ""; // 当前播放的视频id
        this.parentId = queryObj && queryObj.parentId ? queryObj.parentId : ""; // 视频组id
        this.mediaResId =
          queryObj && queryObj.mediaResId ? queryObj.mediaResId : ""; //播放单个视频时需要的的VIDEO-MEDIA_RESOURCEID
        this.resType =
          queryObj && queryObj.resType ? queryObj.resType : "VIDEO-MEDIA";
      }
      if (
        this.resType === "AUDIO-MEDIA" &&
        this.CONFIG.getResourceLists.typeAdapter1
      ) {
        ...
        this.resType === "ZILIAOKU" &&
        this.CONFIG.getResourceLists.typeAdapter2
      ) {
        this.keys = getFieldAdapter(
          this.CONFIG.getResourceLists.sysAdapter,
          this.CONFIG.getResourceLists.typeAdapter2
  ...
  methods: {
    getDetailISBUY() {
      if (
        this.CONFIG.getResourceDetail !== undefined &&
        this.modulename !== "videoplay"
      ) {
        ...
        Get(
          CONFIG.BASE_URL +
            this.CONFIG.getResourceDetail.url +
            "?pubId=" +
            params.pubId +
            "&loginName=" +
            params.loginName
    ...
    getListOrPlayVideo() {
      this.getDetailISBUY();
      if (this.CONFIG.queryParamsType == "") {
        // 从配置里面去参数
        this.queryResourceLists();
        return false;
      }
      if (!this.parentId) {
        //没有parentId表示非视频组，那么播放单个视频
    ...
    initDPlayer(
      url = this.playUrl +
        (this.resType == "ZILIAOKU"
          ? this.curAttachObj[this.keys.fileRecordID]
          : this.curVideoObj[this.keys.resourceId])
    ) {
      // 初始化视频播放器
      let options = {
      ...
      if (this.jwPlayer) {
        ...
        this.jwPlayer = jwplayer("myDPlayer").setup(options);
      }
    },
    initSwiper() {
      // 初始化轮播图
    ...
    toCustomFun(config) {
      // 执行自定义事件
      window.open(
        toOtherPage(this.curVideoObj, this.CONFIG[config.method], this.keys)
      );
    },
    getDetailById(id) {
      //获取当前播放视频的详情信息
    ...
    emitDetailEvent(detail) {
      let eventName = this.CONFIG.event
        ? this.CONFIG.event.emitDetail_name
