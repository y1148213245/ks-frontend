...
var actions = {
  ...
  queryUser({commit}, param) {
    commit('setMember', {loginName: '1'});
          api.queryUser('1').then(function (response) {
    ...
    //     commit('setMember', rep.data['member']);
    //     /*账户信息加载*/
    //     api.queryUser(rep.data['member'].loginName).then(function (response) {
    //       var data = response.data.data;
    //       var account = {
    //         "loginName": data.loginName,
    //         "email": data.email,
  ...
  commitOrder({commit}, params) {
    api.commitOrder(params.param, ({commitInfo}) => commit('setcommitOrder', {
  ...
  queryCoupons({commit}, params) {
    api.queryCoupons(params, ({couponsList}) => commit('setCoupons', couponsList))
  },

  /*查询优惠券可用/已用总金额*/
  querySumCoupons({commit}, params) {
    api.querySumCoupons(params, ({sumCoupons}) => commit('setSumCoupons', sumCoupons))
  },

  /*优惠码激活*/
  queryActiveCoupons({commit}, datas) {
    api.checkCoupons(datas.param, ({checkStatus}) => commit('setActiveCoupons', {
