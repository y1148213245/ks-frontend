...
export default {
	...
	setPasswordByMobile(params){
		var url =BASE_URL+'/user/setPasswordByMobile.do?mobileNum=' + params.mobileNum + '&password=' + params.checkPass;
		return axios.post(url);
	},
	// 注册账号---------------------------------------------------------------------------------------------------------------------------------------
	/*邮箱注册账号*/
	...
	registerBindMobileNum(params){
		var url =BASE_URL+'/user/registerBindMobileNum.do?mobileNum=' + params.mobileNum + '&loginName=' + params.loginName + '&passWord=' + params.checkPass;
		return axios.get(url);
	},
	/*头像上传*/
    ...
    axios.get(BASE_URL+'/order/submitSplitOrder1.do?parentOrderCode=' + params.parentOrderCode).then(function (response) {
      if (cb && cb instanceof Function) {
		...
		/*发送手机验证码*/
		sendToMobile(params){
			var url =BASE_URL+'/user/sendToMobile.do?mobileNum=' + params.mobileNum;
			return axios.get(url);
		},
		/*个人中心设置手机号*/
		bindMobile(params){
			var url =BASE_URL+'/user/bindMobile.do?loginName='+ params.loginName+'&mobileNum=' + params.mobileNum;
			return axios.get(url);
		},
		/*个人中心更换手机号*/
		changeBindMobile(params){
			var url =BASE_URL+'/user/changeBindMobile.do?loginName='+ params.loginName+'&mobileNum=' + params.mobileNum;
			return axios.get(url);
		},
		/*个人中心随手记列表*/
		searchNoteList(params){
			var url =BASE_URL+'/spc/prodb/search.do?doclibCode=PORTAL_NOTE&loginName='+params.loginName+'&status=0&page='+params.page+'&size='+params.size;
			return axios.get(url);
		},
		// 随手记上传
		saveNote(params){
			var url =BASE_URL+'/spc/prodb/saveItem.do';
			return axios.post(url, params);
		},
		// 随手记删除
		deleteNote(params){
			var url =BASE_URL+'/spc/prodb/deleteItem.do?doclibCode=PORTAL_NOTE&DocIDs='+params.DocIDs+'&loginName=' + params.loginName;
			return axios.get(url);
		},
		//随手记详情页
		fetchArticle(params){
			var url =BASE_URL+'/spc/prodb/doc/detail.do?doclibCode=PORTAL_NOTE&docID='+params.DocIDs+'&loginName=' + params.loginName;
			return axios.get(url);
		},
  /**
   * 个人中心 根据用户名获取优惠券
