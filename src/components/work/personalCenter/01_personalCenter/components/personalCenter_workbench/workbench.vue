...
export default {
  ...
  methods: {
    ...
    addRadioComponent() {
      ...
      for (let index = 0; index < awardList.length; index++) {
        ...
        html += `
          ...
          <el-radio-group v-model="vmodel.${"data" +
            item.id}.val" @change="radioClick">
    ...
    toTaskList(row) {
      ...
      this.activityId = row
        ? row[this.config.keys_activity.id]
        : this.activityId;
      ...
      this.$nextTick(() => {
        this.$refs["taskPaging"].changeCurrentPage(
          this.seacrhTaskConditions[this.taskStatus].page - 0,
          this.seacrhTaskConditions[this.taskStatus].size - 0
        );
      });
    },
    /* 任务列表点击事件 */
    ...
    toProductList(row) {
      ...
      $("#radioCom").html("");
      this.$nextTick(() => {
        this.$refs["workPaging"].changeCurrentPage(
          this.seacrhWorkConditions[this.workSearch.awardStatus].page - 0,
          this.seacrhWorkConditions[this.workSearch.awardStatus].size - 0
        );
      });
    },
    /* 去评奖 */
    toReview(row) {
      ...
      this.productDocId = row
        ? row[this.config.keys_product.workId]
        : this.productDocId;
    ...
    getActivityDetail() {
      ...
      Get(CONFIG.BASE_URL + "spc/prodb/detail.do", { params }).then(resp => {
        ...
        let classlimtValue = this.activityDetail[
          this.config.keys_activity.class
        ].split(";");
        let classlimtArr = [];
    ...
    getRatio() {
      ...
      Get(
        CONFIG.BASE_URL + "spc/prodb/activity/api/activity/getRatioByTaskID.do",
        { params }
      ).then(resp => {
    ...
    getTaskInfo() {
      ...
      Get(CONFIG.BASE_URL + "spc/prodb/detail.do", {
        ...
        this.areaListOptions = [...new Set(areaArr)];
        this.schoolOptions = [...new Set(schoolArr)];
    ...
    getDistributeWorkList() {
      ...
      let params = {
        ...
        vals: this.taskId,
        page:
          this.seacrhWorkConditions[this.workSearch.awardStatus].page - 1 + "",
    ...
    addWorkSearchParams(params) {
      let conditions = JSON.parse(
        JSON.stringify(this.seacrhWorkConditions[this.workSearch.awardStatus])
      );
    ...
    tabClick(tab, event) {
      let conditions = JSON.parse(
        JSON.stringify(this.seacrhWorkConditions[tab.name])
      );
        ...
        if (resp.data.content.length > 0) {
          ...
          if (distributeWorkList.length > 0) {
            ...
            if (distributeWorkList[0][this.config.keys_product.award]) {
              awardsArr = distributeWorkList[0][
                this.config.keys_product.award
              ].split(",");
            } else if (
              this.productDetail.hasOwnProperty(
                this.config.keys_productDetail.award
              ) &&
              this.productDetail[this.config.keys_productDetail.award]
            ) {
              awardsArr = this.productDetail[
                this.config.keys_productDetail.award
              ].split(",");
            }
    ...
    getAwardList() {
      ...
      Post(CONFIG.BASE_URL + "spc/prodb/searchNLP.do", params).then(resp => {
        ...
        for (let index = 0; index < arr.length; index++) {
          ...
          awardListOptions.push(
            element[_this.config.keys_award.type] +
              ":" +
              element[_this.config.keys_award.topic]
          );
        ...
        this.awardTypeArr = [...new Set(awardTypeArr)];
    ...
    getDistributeWorkPrevNext() {
      ...
      Post(CONFIG.BASE_URL + "spc/prodb/searchNLPPrevNext.do", params).then(
        resp => {
    ...
    getProductDetailByRecordId() {
      ...
      Post(CONFIG.BASE_URL + "spc/prodb/searchNLP.do", params).then(resp => {
        if (resp.data.content.length > 0) {
          ...
          this.productDocId =
            resp.data.content[0][this.config.keys_product.workId];
          this.toReview();
        }
      });
    },
    formFilter() {
      ...
      this.seacrhWorkConditions[this.workSearch.awardStatus].areaList =
        conditions.areaList;
      this.seacrhWorkConditions[this.workSearch.awardStatus].school =
        conditions.school;
      this.seacrhWorkConditions[this.workSearch.awardStatus].group =
        conditions.group;
      this.seacrhWorkConditions[this.workSearch.awardStatus].workName =
        conditions.workName;
      this.seacrhWorkConditions[this.workSearch.awardStatus].awardList =
        conditions.awardList;
