  ...
  <div class="tac personal_center_nav">
    ...
    <el-menu :default-active="currentShowIndex" class="el-menu-vertical-demo" @open="handleOpen" @close="handleClose">
      <template v-for="(entry,index) in navs">
        <el-menu-item :index="String(index)" class="personal_center_nav_entry" @click="showComponent(index)" :key="index"><i
...
export default {
  ...
  mounted() {
    ...
    if (hash) {
      ...
      let nav = this.navs.filter((item)=>{
          return item.tag == tag
      })
      this.updateCurrentShow(nav[0]);
    } else {
      this.updateCurrentShow(this.navs[0]);
    }
