...
<template>
	<div class="work_shoppingcart_05">
		<div class="work_shoppingcart_05_main">
			<template v-for="(config, config_i) in resourceDetailConfig.complicatedItem" v-if="resourceDetail">
				<!-- 需要特殊处理的复杂项 -->
				<!-- img 图片 -->
				<div :key="config_i" v-if="config.name == 'img'" class="work_shoppingcart_05_imgcontainter" @click="toCustomFun(resourceDetail, config)">
					<label class="work_shoppingcart_05_img_label">{{config.display}}</label>
					<img class="work_shoppingcart_05_img" v-bind="{class: 'work_shoppingcart_05_' + config.field}" :src=" resourceDetail[keys[config.field]] " alt="暂无图片" @load="dealResourceImg($event)" />
				</div>

				<!-- 自定义事件按钮 包括（title 标题） -->
				<div :key="config_i" v-else-if="config.name == 'button'" v-bind="{class: 'work_shoppingcart_05_btncontainer_' + config.field + '_' + config_i}" @click="toCustomFun(resourceDetail, config)">
					<i v-bind="{class: config.className}"></i>
					<label class="work_shoppingcart_05_btnlabel">{{config.display}}</label>
					<span v-bind="{class: 'work_shoppingcart_05__btncontainer_' + config.field}" v-if="keys[config.field]">{{ resourceDetail[keys[config.field]] }}</span>
				</div>

				<!-- price 价格 -->
				<div :key="config_i" v-else-if="config.name == 'price'" class="work_shoppingcart_05_pricecontainter">
					<label class="work_shoppingcart_05_price_label">{{config.display}}</label>
					<span v-bind="{class: 'work_shoppingcart_05_' + config.field}">{{ resourceDetail[keys[config.field]]  | formatPriceNew }}</span>
				</div>

				<!-- time 时间 -->
				<div :key="config_i" v-else-if="config.name == 'time'" class="work_shoppingcart_05_timecontainter">
					<label class="work_shoppingcart_05_time_label">{{config.display}}</label>
					<span v-bind="{class: 'work_shoppingcart_05_' + config.field}">{{ resourceDetail[keys[config.field]] | formatDateNEW}}</span>
				</div>

				<!-- 其他不需要特殊处理的简单项 -->
				<div :key="config_i" v-else class="work_shoppingcart_05_other">
					<i v-bind="{class: config.className}"></i>
					<label class="work_shoppingcart_05_label">{{config.display}}</label>
					<span v-if="config.field" v-bind="{class: 'work_shoppingcart_05_' + config.field}" v-html="resourceDetail[keys[config.field]] || '暂无'"></span>
				</div>
			</template>
		</div>
		...
		<div class="work_shoppingcart_05_commitOrder">
			<el-button class="work_shoppingcart_05_commitOrder_button" @click="commitOrder()">购 买</el-button>
		</div>
...
<style>
	...
	.work_shoppingcart_05_imgcontainter {
