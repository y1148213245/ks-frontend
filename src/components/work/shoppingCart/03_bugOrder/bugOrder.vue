...
export default {
  ...
  mounted() {
    ...
    this.CONFIG =
      PROJECT_CONFIG[this.namespace].shoppingCart.shoppingCart_03[
        this.modulename
      ];
  ...
  methods: {
    ...
    toCustomFun(resourceDetail, config) {
      // 执行自定义事件
      window.open(
        toOtherPage(this.resourceDetail, this.CONFIG[config.method], this.keys)
      );
    },
    getResourceDetail() {
      //获取商品详情
    ...
    commitOrder() {
      ...
      if (this.resourceDetail.isBuy == "1") {
        ...
        Post(
          CONFIG.BASE_URL + "order/submitSplitOrder.do",
          this.Orderparams
          ...
          if (response.status == 200 && response.data) {
            ...
            if (datas.data && datas.result == 1 && datas.data.submitStatus) {
              if (_this.defaultPay == "Balance") {
                //余额支付
                location.href =
                  _this.orderSuccessUrl +
                  "?pubId=" +
                  _this.pubId +
                  "&&loginName=" +
                  _this.loginName;
              } else if (_this.defaultPay == "Weixin") {
                //微信支付
                loading.close();
                let params = {
                ...
                Get(CONFIG.BASE_URL + "epay/getWxShopPay.do", {
                  params: params
                }).then(resp => {
                  let datas = resp.data;
                  if (datas.result == "1") {
    ...
    setOrderParams() {
      ...
      this.Orderparams = {
        ...
        orderCode: "", //不用写
        balanceAmount: this.defaultPay == "Balance" ? this.bookMoney : 0, // 如果是余额支付，那就写支付金额 不是就写0
        createTime: null, //不用写
        ...
        payUser: "", //不用写
        realAmount: this.defaultPay == "Balance" ? 0 : this.bookMoney, // 如果是余额支付，那写0 不是写支付金额
