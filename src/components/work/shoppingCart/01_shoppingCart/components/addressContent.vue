...
<template>
    <div class="work_shoppingcart_01_address">
        ...
        <el-dialog title="新增收货地址" :visible.sync="addAddressDialog" class="newAddAddress" :close-on-press-escape="false"
                   :close-on-click-modal="false" @close="newAddAddressClose">
          ...
          <div class="newWrapper">
            <div>联系电话：</div>
            <input id="s_phone" type="text" @blur="checkPhone()"
...
<script>
    ...
    export default {
        ...
        methods: {
            loadCallBack() {
                this.$store.dispatch("shoppingcart/" + type.QUERY_ORDER_ADDRESS, this.member.loginName);    // 查询收货地址列表
                this.$store.dispatch("shoppingcart/" + type.QUERY_DEFAULT_ADDRESS, this.member.loginName);  // 查询默认收货地址
            },
            ...
            addNewAddress: function() {  // 新增地址
                this.addAddressDialog = true;
                initDom();

                function initDom() {
                    setTimeout(function() {
                        if (document.getElementById("s_province")) { //解决弹框出来的时候DOM可能还没有加载完成问题
                            _init_area(document);
                        } else {
                            initDom();
                        }
            ...
            selectAddress: function(address) { // 在模态弹窗中选中某个地址
                this.tempAddress = JSON.parse(JSON.stringify(address));
            },
            confirmAdd: function(flag) {    // 确认/取消添加收货地址
            ...
            confirmNewAdd: function(flag)  {  // 确认/取消新增地址
                var _this = this;
                if (flag) { // 点击确定
                    if ($("#s_contactor").val() === "") { // 收件人为空
                        this.emptyContactor = true;
                        return false;
                    } else if ($("#s_province").val() === "省份" || $("#s_city").val() === "地级市" || $("#s_county").val() === "市、县级市") { // 省市区没有选择或者没有选择完全
                        this.emptyPCC = true;
                        return false;
                    } else if ($("#s_address").val() === "") { // 详细地址为空
                        this.emptyDetail = true;
                        this.emptyPCC = false;
                        return false;
                    } else if ($("#s_phone").val() === "") { // 联系方式为空
                        ...
                        var requestParams = {
                            ...
                            myCallback: function() {
                                if (this.addStatus) {
                                    ...
                                    _this.$store.dispatch("shoppingcart/" + type.QUERY_DEFAULT_ADDRESS, _this.member.loginName);
                                    _this.$store.dispatch("shoppingcart/" + type.QUERY_ORDER_ADDRESS,_this.member.loginName);
                                } else {
                                    _this.$message({
                                        type: "error",
                                        message: "新增地址失败"
                    ...
                    this.$store.dispatch("shoppingcart/" + type.ADD_ORDER_ADDRESS, requestParams);
                    }
                }
            ...
            checkContactor: function() {  // 联系人失去焦点校验
                this.emptyContactor = $("#s_contactor").val() === "" ? true : false;
            },
            checkDetail: function() {   // 详细地址失去焦点校验
                this.emptyDetail = $("#s_address").val() === "" ? true : false;
            },
            checkPhone: function() {   // 联系方式失去焦点校验
                this.emptyPhone = $("#s_phone").val() === "" ? true : false;
            },
            // 后续改成输入完之后再校验 不实时校验
            checkNumberType: function(event) { // 联系号码格式校验 只能是数字 并且不能超过11位 ??? input type="number" 踩坑
                if (!String.fromCharCode(event.keyCode).match(/\d/)) {  // 控制只能输入数字
