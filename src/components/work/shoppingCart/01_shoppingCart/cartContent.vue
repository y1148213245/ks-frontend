    ...
    <div id="cartWrapper" class="main_car" v-show="showItem == 'showCartWrapper'" v-loading="loading">
      ...
      <div v-if="productList && productList.length>0">
        ...
        <div class="container" id="priceChange" v-if="priceChangeList.length !== 0">
          ...
          <div v-for="(info, infoInd) in priceChangeList" :key="infoInd">
        ...
        <div class="container cartContainer">
          <div id="productListWrapper">
            ...
            <div class="cart-item">
              ...
              <!-- product list -->
              <ul class="cart-item-list" v-for="(item, Oindex) in productList" :key="Oindex" :class="{selectedCombinateProduct: item.checked}">
                <div class="occupyHeightContainer" style="height: 30px;background-color: #fff;"></div><!-- 占高容器 -->
                <!-- 2018/08/27 新增组合购买：只有一个删除按钮 只能同种类型商品组合 -->
                <template v-if="item.isCombinateProduct == 'yes'">
                  <li class="combinateProductHead bookWrapper">
                    <div class="cart-tab-1">
                      <!-- 按钮 -->
                      <div class="cart-item-check">
                        <el-checkbox v-model="item.checked" @change="selectedProduct(null, item)"></el-checkbox>
                      </div>
                    </div>
                    <div class="cart-tab-2">
                      <div class="item-name">
                        <span v-text="item.combinateName"></span>
                      </div>
                    </div>
                    <div class="cart-tab-3">
                      <!-- 价格 和 活动 -->
                      <div class="item-price">
                        <span>{{formatMoney(item.combinatePrice)}}</span>
                      </div>
                    </div>
                    <div class="cart-tab-4">
                      ...
                      <div class="quantity" v-if="item.productType == bookTypeTag">
                        ...
                        <input class="productNums" type="number" v-model="item.nums" @keyup="changeQuantity(item, 0, $event)" @keypress="checkNumber($event, item)">
                        <a href="javascript:void(0)" v-on:click="changeQuantity(item,1)">+</a>
                      </div>
                      <!-- 电子书没有步进器 -->
                      <div class="quantity" v-if="item.productType == ebookTypeTag">
                        <span v-text="item.nums"></span>
                      </div>
                    </div>
                    <div class="cart-tab-5">
                      <!-- 总价 -->
                      <span> {{(Math.round(item.combinatePrice * item.nums * 100) / 100).toFixed(2)}}</span>
                    </div>
                    <div class="cart-tab-6">
                      <!-- 删除 、收藏等操作 -->
                      <div class="cartOpration">
                        <div class="delete opertion" @click="deleteProduct(item)">
                  ...
                  <li v-for="(product, index) in item.list" class="combinateProductList bookWrapper" v-bind:class="{checkedLi: product.checked}" :key="index">
                    <template class="combinateProductLists">
                      <div class="cart-tab-1">
                        <!-- 按钮 -->
                      </div>
                      <div class="cart-tab-2">
                ...
                <template v-else>
                  ...
                  <li v-for="(product, index) in item.list" v-if="item.productType == bookTypeTag || item.productType == seriesTypeTag" class="bookWrapper" v-bind:class="{checkedLi: product.checked}" :key="index">
                    ...
                    <div class="cart-tab-3">
                      ...
                      <select name="" v-if="product.productDiscountList.length > 0" style="max-width:350px;" @change="selectActivity($event, product, item)">
                        ...
                        <option v-for="(item, ind) in product.productDiscountList" v-bind="{selected: item.id === product.activityId ? 'selected' : false, value: item.id}" :key="ind">
                    ...
                    <div class="cart-tab-4">
                      <div class="quantity">
                        ...
                        <input class="productNums" type="number" v-model="product.nums" @keyup="changeQuantity(product, 0, $event)" @keypress="checkNumber($event, product)">
                        <a href="javascript:void(0)" v-on:click="changeQuantity(product,1)">+</a>
                      </div>
                    </div>
                    <div class="cart-tab-5">
                  ...
                  <li v-for="(product, index) in item.list" v-if="item.productType == ebookTypeTag || item.productType == ejournalTypeTag" class="ebookWrapper" v-bind:class="{checkedLi: product.checked}" :key="index">
                    <div class="cart-tab-1">
                      <div class="cart-item-check">
                        <el-checkbox v-model="product.checked" @change="selectedProduct(product, item)"></el-checkbox>
                      </div>
            ...
            <div id="clearingWrapper">
              ...
              <div class="clearing">
                <div class="zeroTips" v-if="showZeroTips">
                  <span></span>
                  {{getStaticText('selectAtLeastOne') ? getStaticText('selectAtLeastOne'): '请选择至少一件商品'}}
                </div>
              ...
              <div class="selectedProduct">
                <span>{{getStaticText('haveSelected') ? getStaticText('haveSelected') : '已选中'}}
                  <span class="selectedProductCount" v-text="totalNum">0</span>{{getStaticText('productQuanity') ? getStaticText('productQuanity'): '件商品'}}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="container">
        ...
        <div class="discount" v-if="isShowCoupon">
          ...
          <div class="myCouponsWrapper">
            <div class="disabledCoupons" v-for="(list, index) in couponsList" v-if="couponsList.length > 0" :class="{'myCoupons': list.isAvailablesm === true, 'activeCoupons': selectedCouponsPassword === list.password}" @click="selectCoupon(list, index)" @mouseenter="showCancelBtn(list.password)" @mouseleave="hideCancelBtn(list.password)" :key="index">
        ...
        <div class="payremark">
          ...
          <div class="orderContent">
            ...
            <p>{{getStaticText('youCanAlsoInput') ? getStaticText('youCanAlsoInput') : '您还可以输入'}}
              <span>{{50-payremark.length}}</span>{{getStaticText('wordQuanity') ? getStaticText('wordQuanity') : '个字'}}</p>
          </div>
        </div>
        ...
        <div class="selectedProductList">
          ...
          <!--order product list-->
          <div class="cart-item">
            <ul class="cart-item-list" v-for="(item, index) in orderList" v-if="item.list.length>0" :key="index">
              <div class="occupyHeightContainer" style="height: 30px;"></div><!-- 占高容器 -->
              <!-- 2018/08/27 新增组合购买 -->
              <template v-if="item.isCombinateProduct == 'yes'">
                <li class="combinateProductHead bookWrapper">
                  <div class="cart-tab-1">
                ...
                <li v-for="(product, index) in item.list" class="bookWrapper" v-bind:class="{checkedLi: product.checked}" :key="index">
                  <template class="combinateProductLists">
                    <div class="cart-tab-1">
                      <div class="cart-item-pic">
                        <img v-bind:src="product.smallPic || '../assets/img/zwfm.png'" onload="DrawImage(this,80,80)">
              ...
              <template v-else>
                ...
                <li v-for="(product, innerIndex) in item.list" :key="innerIndex">
                  <div class="cart-tab-1">
                    <div class="cart-item-pic">
                      <img v-bind:src="product.smallPic || '../assets/img/zwfm.png'" onload="DrawImage(this,80,80)">
                    </div>
        ...
        <div class="orderFooter">
          <div class="amount">
            ...
            <span>{{getStaticText('shouldPay') ? getStaticText('totalCount') : '应付：'}}{{formatMoney(orderDetail.totalMoney + orderDetail.saveAmount)}}</span>
          </div>
          <div class="virtual">
...
export default {
  ...
  mounted: function () {
    if (window.location.hash) { // #/myOrder/loginName  提交订单页面 即在提交订单页面执行刷新操作
      this.getRecordOrder();
    } else {                    // 购物车列表页面 即 结算页面
    ...
    let hasTop = false;
    $(window).scroll((event) => {               // 监控滚动条 控制下方结算按钮要始终在可视区域
  ...
  computed: {
    ...
    recordOrder: function () { // 将订单商品列表从本地存储中取出来（刷新的时候）
      var tempList = [];
  ...
  methods: {
    ...
    getRecordOrder: function () {  // 在提交订单也执行刷新的时候执行  取存在 sessionStorage 里面的东西
      ...
      this.showItem = window.sessionStorage.getItem("showItem"); // 显示状态 是列表页 or 提交订单页
      sessionOrderList.forEach((item) => {
        this.orderList.push(item);
      });
      sessionOrderList.forEach((items) => {
        if (items.list.length > 0) {
          this.selectedOrderList.push(items);
        }
      });
    ...
    commonMessageFun(obj, status, configText, defaultText) {
      obj.$message({
        type: status,
        message: obj.getStaticText(configText) ? obj.getStaticText(configText) : defaultText
      });
    },
    deleteProduct: function (id) {  // 删除商品
      ...
      this.$confirm(_this.getStaticText('areYouSureToDeleteTheItem') ? _this.getStaticText('areYouSureToDeleteTheItem') : "您确定要删除该商品吗?", _this.getStaticText('systemPrompt') ? _this.getStaticText('systemPrompt') : "系统提示", {
        ...
        if (id.isCombinateProduct === 'yes') { // 组合购买删除的时候传递的是整个对象
        ...
        } else { // 删除非组合购买商品的时候 传递当前要删除的单个商品对象
          var param = {
            ids: id,
            cb: function () {
    ...
    selectAll: function () {       // 商品全选操作
      ...
      for (var i = 0; i < data.length; i++) {
        if (data[i].isCombinateProduct === 'yes') { // 组合购买商品
          data[i].checked = this.selectedAll;
        } else { // 非组合购买商品
      ...
      data.forEach((data) => {
        ...
        Vue.set(data, "selectedOne", this.selectedAll);
      });
      this.calcTotalMoney();
    },
    ...
    calcTotalMoney: function () { // 计算选中商品总价（不参加任何活动的单纯计算）
        ...
        if (data[i].isCombinateProduct === "yes" && data[i].checked) { // 组合购买商品 独立的模块 不涉及商品活动和优惠券
          ...
          for (var j = 0; j < dataList.length; j++) {
            if (dataList[j].checked) {
              ...
              var list = this.selectedProductList["activity" + dataList[j].activityId + dataList[j].productType + dataList[j].combinateId].list; // 把当前勾选中的商品放到他的活动id对应的对象里的list数组里面
                ...
                switch (discountType) {
                  case "cutMoney": // 满多少钱减多少钱
                    ...
                    if (dataList[j].discountRule === "fullReduce") { // 一级活动分类：满多少钱 + 4种操作 减钱
                      if (curTotalMoney >= referAmount) { // 满足满多少钱减多少钱的条件
                        this.selectedProductList["activity" + dataList[j].activityId + dataList[j].productType + dataList[j].combinateId].saveValue = discountAmount;
                      }
                    } else if (dataList[j].discountRule === "fixedDiscount") { // 一级活动分类：固定折扣 减钱
                      this.selectedProductList["activity" + dataList[j].activityId + dataList[j].productType + dataList[j].combinateId].saveValue = discountAmount > curTotalMoney ? curTotalMoney : discountAmount;
                    }
                  ...
                  case "discount": // 满多少钱打折
                    ...
                    if (dataList[j].discountRule === "fullReduce") { // 一级活动分类：满多少钱 + 4种操作 打折
                      if (curTotalMoney >= referAmount) { // 满足满多少钱打折的条件
                        ...
                        this.selectedProductList["activity" + dataList[j].activityId + dataList[j].productType + dataList[j].combinateId].saveValue = 0;
                      }
                  ...
                  case "sendPoints": //满多少送积分  待定
                    ...
                    if (dataList[j].discountRule === "fullReduce") {
                      ...
                      if (curTotalMoney >= referAmount) {
                        // 满足满多少钱送积分的条件
                        this.selectedProductList["activity" + dataList[j].activityId + dataList[j].productType + dataList[j].combinateId].sendPoints = points;
                      } else {
                        this.selectedProductList["activity" + dataList[j].activityId + dataList[j].productType + dataList[j].combinateId].sendPoints = 0;
                      }
                    } else if (dataList[j].discountRule === "fixedDiscount") {
                      // 一级活动分类：固定折扣 送积分
                      this.selectedProductList["activity" + dataList[j].activityId + dataList[j].productType + dataList[j].combinateId].sendPoints = points;
      ...
      this.saveAmount = saveMoneyArray.length > 0 ? eval(saveMoneyArray.join("+")) : 0; // 节省金额
      this.bookTotalMoney = bookTotalMoneyArray.length > 0 ? eval(bookTotalMoneyArray.join("+")) : 0; // 纸质商品总价
      this.bookSaveMoney = bookSaveMoneyArray.length > 0 ? eval(bookSaveMoneyArray.join("+")) : 0; // 纸质商品节省金额
      this.ebookTotalMoney = ebookTotalMoneyArray.length > 0 ? eval(ebookTotalMoneyArray.join("+")) : 0; // 电子商品总价
      this.ebookSaveMoney = ebookSaveMoneyArray.length > 0 ? eval(ebookSaveMoneyArray.join("+")) : 0; // 电子商品节省金额
      this.sendPoints = sendPointsArray.length > 0 ? eval(sendPointsArray.join("+")) : 0; // 赠送积分
      /* console.log('totalMoney === this.bookTotalMoney + this.ebookTotalMoney', totalMoney === this.bookTotalMoney + this.ebookTotalMoney); // true 表明没有算错
      console.log('this.saveAmount === this.bookSaveMoney + this.ebookSaveMoney', this.saveAmount === this.bookSaveMoney + this.ebookSaveMoney); // true 表明没有算错 */
      this.totalMoney = totalMoney - this.saveAmount; // 已选列表商品的总价格
    ...
    calSaveMoney: function () {   // 计算节省金额
      ...
      this.productList.forEach(function (item) {
        ...
        _this.selectedProductList["activity" + item.activityId + item.productType + item.combinateId] = {
          list: [],
          saveValue: 0, // 默认初始值 节省 0 元
          sendPoints: 0, // 送积分 默认值 0 积分
          totalPrice: 0, // 总价 需要计算
          productType: item.productType, // 商品组类型
          activityName: item.activityName, // 活动名称
          activityId: item.activityId, // 活动id
          combinateName: item.combinateName, // 组合购买名称
          combinatePrice: item.combinatePrice, // 组合购买价格
          isCombinateProduct: item.isCombinateProduct, // 是否是组合购买
          nums: item.nums, // 组合购买套数
        }
        _this.selectedProductListArray.push(_this.selectedProductList["activity" + item.activityId + item.productType + item.combinateId]);
      });
    },
    ...
    changeQuantity: function (product, val, e) { // 改变纸质书商品数量
      ...
      this.calcTotalMoney();
      var _this = this;
      if (product.isCombinateProduct === "yes") {
        axios.get(CONFIG.BASE_URL + 'cart/updateCombinationProductCart.do' + '?combinationId=' + product.combinateId + '&number=' + product.nums + '&loginName=' + this.member.loginName).then((res) => { // 更新组合购买的数量
          if (res.data && res.data.result == '1') { // 更新成功 再请求一次购物车刷新组合购买内部商品的数量
            // _this.$store.dispatch("shoppingcart/" + type.QUERY_SHOPPING_CART,{param: { loginName: _this.member.loginName}})
          } else { //  更新失败
            _this.$message({
              type: "error",
              message: _this.getStaticText('operateError') ? _this.getStaticText('operateError') : "操作失败"
            });
          }
        })
      } else {
    ...
    checkNumber: function (event, product) { // 购买数量格式校验
      if (!String.fromCharCode(event.keyCode).match(/\d/) || $(".productNums").val().length > 3) { // 不大于三位数的纯数字
      ...
      var params = {
        ...
        myCallbacks: function () {
          if (this.rmbCoin) {  // 下载币兑换成功
            ...
            if (this.rmbCoin < 0) {
              _this.balancePay = false;
              _this.$alert(_this.getStaticText('downloadCoinDiscountCanNotBeLessThanZero') ? _this.getStaticText('downloadCoinDiscountCanNotBeLessThanZero') : "下载币优惠数额不能小于0", _this.getStaticText('systemPrompt') ? _this.getStaticText('systemPrompt') : "系统提示", {
                confirmButtonText: _this.getStaticText('confirm') ? _this.getStaticText('confirm') : "确定"
              });
            ...
            if (this.rmbCoin.toFixed(2) > payAmount) {
              ...
              _this.$alert(_this.getStaticText('downloadCoinDiscountCanNotBeMoreThanAmountActuallyPaid') ? _this.getStaticText('downloadCoinDiscountCanNotBeMoreThanAmountActuallyPaid') : "下载币优惠数额不得大于实付金额", _this.getStaticText('systemPrompt') ? _this.getStaticText('systemPrompt') : "系统提示", {
                confirmButtonText: _this.getStaticText('confirm') ? _this.getStaticText('confirm') : "确定"
              });
    ...
    formatMoney: function (value) { // 价格过滤器
      if (value) {
...
/* 组合购买样式 */
#productListWrapper .selectedCombinateProduct {
  background-color: #fffaf7;
}

.cart-item .cart-item-list::after {
  content: ' ';
}

.cart-item-list .combinateProductList > div {
  border-bottom: none;
  border-top: none;
}

.cart-item-list .combinateProductList:last-child > div {
